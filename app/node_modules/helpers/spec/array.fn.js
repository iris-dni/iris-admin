...
describe('helpers/array', () => {
  ...
  describe('removeValuesFilter', () => {
    it('creates a (filter) function', () => {
      assert.isFunction(removeValuesFilter(['a', 'b']));
    });

    it('rejects the specified values', () => {
      const array = ['a', 'b', 'c', 'a', 'c', 'c', 'b', 'a', 'd'];
      const values = ['a', 'd'];
      const expected = ['b', 'c', 'c', 'c', 'b'];
      assert.deepEqual(array.filter(removeValuesFilter(values)), expected);
    });
  });

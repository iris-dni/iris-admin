...
const createApi = (apiName) => ({
  ...
  get(id, relationsToResolve) {
    return new Promise((resolve, reject) => {
      api(apiName).get({ contentId: id, resolve: relationsToResolve })
        .then((response) => {
          if (response && response.obj) {
  ...
  update(id, data, relationsToResolve) {
    return new Promise((resolve, reject) => {
      api(apiName).update({ contentId: id, data: { data: data }, resolve: relationsToResolve })
        .then((response) => {
          if (response && response.obj) {
            resolve({ data: response.obj.data });
          } else {

...
const relationsToResolve = ['images', 'links', 'mentions', 'owner'];
...
export default withRouter(makePage(React.createClass({
  ...
  fetchPetition(id) {
    PetitionAPI.get(id, relationsToResolve).then((response) => {
      this.setState({
        dirty: [],
        error: null,
  ...
  save(keys) {
    return new Promise((resolve, reject) => {
      ...
      PetitionAPI.update(petition.id, partialPetition, relationsToResolve).then(response => {
  ...
  checkTransitionVetos(transitionName) {
    ...
    if (transitionName === 'setFeedback') {
      if (!petition.city_answer || !petition.city_answer.text || !petition.city_answer.name) {
        return missingProps(['city_answer.text', 'city_answer.name'])
      }
      if (!petition.response_token) {
        return missingProps(['response_token'])
      }
    }

    if (transitionName === 'approved' && !petition.city.id) {
    ...
    return (
      <div className="container-fluid">
        ...
        <BasicInfo
          ...
          />
        <ImageBox {...blockProps} />
        <UrlBox {...blockProps} name="links" cardTitle="URLs" />
        <UrlBox {...blockProps} name="mentions" cardTitle="Mentions" />
        <UserInfo {...blockProps} />

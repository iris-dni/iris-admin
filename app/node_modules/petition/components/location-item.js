      ...
      <div className="col-xs-12" style={{ marginBottom: 25 }}>
        ...
        <div style={{ marginLeft: thumbnailSize + 15 }}>
          ...
          <div style={{ marginTop: -5 }}>
            {this.renderButton({ name: 'toggle-visibility', label: item.visible ? 'Hide' : 'Show' })}
            {this.renderButton({ name: 'delete', variation: 'danger', label: 'Delete' })}
            {this.renderButton({ name: 'refetch-og', label: 'Re-Fetch OG' })}
          </div>
        </div>
      </div>
    );

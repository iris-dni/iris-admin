...
import WebLocationAPI from 'weblocation/api';
...
export default React.createClass({
  ...
  resetOG(items, itemId) {
    WebLocationAPI.resetOG(itemId).then(() => {
      this.props.onValueChange(this.props.name, items.map(item => {
        if (item.id !== itemId) {
          return item;
        }
        item.data = Object.assign({}, item.data, {og: null});
        return item;
      }));
    }).catch(() => {
      this.setState({ error: translate('petitions.errors.reset-og') });
    });
  },

...
export default React.createClass({
  ...
  handleAction(action, payload) {
    ...
    switch (action) {
      case 'add-url':
        this.updateItemsAndSave([...items, { url: payload }]);
        break;
      case 'delete':
        this.updateItemsAndSave(items.filter(i => i.url !== payload.url));
        break;
      case 'toggle-visibility':
        this.updateItemsAndSave(items.map(i => {
          if (i.url !== payload.url) {
            return i;
          }
          return Object.assign({}, i, { visible: !i.visible });
        }));
        break;
      case 'refetch-og':

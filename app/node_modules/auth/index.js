 ...
 */
const Auth = (WrappedComponent, allowPublicAccess) => withRouter(React.createClass({
  displayName: 'Auth',
  ...
  componentWillMount() {
    const regExp = new RegExp(/petition\/\w*\/letter\/\w*/);
    if (this.props.location.pathname !== '/petitions' && !regExp.test(this.props.location.pathname)) {
  ...
  render() {
    const { finishedLoading, me } = this.state;

    if (finishedLoading) {
      if (allowPublicAccess || hasAdminAccess(me)) {
        return (
          <WrappedComponent {...this.props} me={me} />
        );
      }
      this.showLoginPage();
    }

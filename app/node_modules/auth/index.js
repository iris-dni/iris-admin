...
const Auth = (WrappedComponent) => withRouter(React.createClass({
  ...
  componentWillMount() {
    ...
    let params = {};
    if (query && query.iristoken) {
      params.token = query.iristoken;
    }
    AuthApi.whoami(params).then(response => {
      let state = {
        me: null,
  ...
  render() {
    if (this.state.finishedLoading) {
      if (hasAdminAccess(this.state.me)) {
        ...
        AuthApi.logout().then(this.showLoginPage).catch(this.showLoginPage);
      } else {
        this.showLoginPage();
      }
    }
